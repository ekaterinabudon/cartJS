...
const fixbudgetButton = document.getElementById('fixbudget');
fixbudgetButton.addEventListener('click', () => {
  updateBudget();
});
...
document.addEventListener('click', (event) => {
  ...
  if (event.target.classList.contains('plus')) {
    ...
    const item = document.querySelector("#cart ul li[data-id='" + id + "']");

    let newPrice = parseFloat(item.querySelectorAll('p')[1].querySelector('span').innerHTML);
...
function updateCart(){
  ...
  for (let i = 0; i < cart_items.length; i++) {
    ...
    let price = document.getElementsByClassName('data-price')[i].innerHTML;
...
function removeCartItem(id) {
  let item = document.querySelector("#cart ul li[data-id='" + id + "']");
  let quantity = item.getAttribute('data-quantity');
  quantity = parseInt(quantity) - 1
  item.setAttribute('data-quantity', quantity);
